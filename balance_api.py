...
class BalanceResult:
    def __init__(self, ok=False, error="", is_available=False, infos=None):
        self.ok = ok
        self.error = error
...
class BalanceApi(QObject):
    ...
    def _on_finished(self, reply: QNetworkReply):
        ...
        if reply.error() != QNetworkReply.NetworkError.NoError:
            if status == 401:
                self.finished.emit(BalanceResult(error="API Key 无效 (401)"))
            elif status == 402:
                self.finished.emit(BalanceResult(error="余额不足 (402)"))
            else:
                self.finished.emit(
                    BalanceResult(error=f"网络错误: {reply.errorString()}")
                )
            reply.deleteLater()
            return
        try:
            ...
            reply.deleteLater()
            self.finished.emit(BalanceResult(error="响应解析失败"))
            return
        reply.deleteLater()
        if status != 200:
            self.finished.emit(
                BalanceResult(error=f"HTTP {status}: {payload.get('error', {}).get('message', '')}")
            )
            return
        infos = []
